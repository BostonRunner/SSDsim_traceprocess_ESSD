#!/usr/bin/env bash
# 顺序跑 + 安全清理（final layout）
# lower(/dev/vdb):  /mnt/docker/lower/c1..c6
# upper per c{i}:   /mnt/docker/upper/c{i}/upper
# work  per c{i}:   /mnt/docker/upper/c{i}/work   (与 upper 同盘、且非彼此子目录)
# merged:           /mnt/docker/merged/c{i}
...
mkdir -p "${RESULT_ROOT}/single" "${MERGED_ROOT}"
for i in $(seq 1 6); do mkdir -p "${RESULT_ROOT}/single/c${i}" "${MERGED_ROOT}/c${i}" "${UPPER_ROOT}/c${i}/"{upper,work} ; done
...
check_mounts() {
  ...
  for i in $(seq 1 6); do
    ...
    # work 必须与 upper 同盘，且非彼此子目录（我们使用并列目录，天然满足）
...
for i in $(seq 1 6); do
  ...
  umount "$merged" || true
  rm -rf "${upper}/_fio" "${upper}/testfile" 2>/dev/null || true
  rm -rf "${merged}/_fio" "${merged}/testfile" 2>/dev/null || true
done
